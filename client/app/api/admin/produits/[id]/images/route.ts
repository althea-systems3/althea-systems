...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    if (currentUser) {
      await logAdminActivity(currentUser.user.id, 'products.images.upload', {
        productId,
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    if (currentUser) {
      await logAdminActivity(currentUser.user.id, 'products.images.update', {
        productId,
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    const hasTargetImage = currentImages.some(
    ...
    if (currentUser) {
      await logAdminActivity(currentUser.user.id, 'products.images.delete', {
        productId,
        imageUrl,
      });
    }
