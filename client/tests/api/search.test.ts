...
describe("GET /api/search", () => {
  ...
  it("filtre explicitement sur le titre", async () => {
    mockSupabaseProducts.mockReturnValue({
      data: ALL_PRODUCTS,
      error: null,
    })

    const response = await GET(createRequest("?title=Switch"))
  ...
  it("filtre explicitement sur la description", async () => {
    mockSupabaseProducts.mockReturnValue({
      data: ALL_PRODUCTS,
      error: null,
    })

    const response = await GET(createRequest("?description=traitement DSP"))
  ...
  it("filtre explicitement sur les caractéristiques techniques", async () => {
    mockSupabaseProducts.mockReturnValue({
      data: ALL_PRODUCTS,
      error: null,
    })

    const response = await GET(createRequest("?characteristics=USB-C"))
  ...
  it("trie par prix ascendant", async () => {
    ...
    expect(body.products[1].priceTtc).toBe(899.0)
    expect(body.products[2].priceTtc).toBe(999.0)
  })
  ...
  it("trie par prix descendant", async () => {
    ...
    expect(body.products[0].priceTtc).toBe(999.0)
    expect(body.products[2].priceTtc).toBe(649.99)
  })

  it("trie par nouveauté descendante", async () => {
    mockSupabaseProducts.mockReturnValue({
      data: ALL_PRODUCTS,
      error: null,
    })
    ...
    const response = await GET(
      createRequest("?sort_by=newness&sort_order=desc"),
    )
  ...
  it("trie par disponibilité ascendante", async () => {
    mockSupabaseProducts.mockReturnValue({
      data: ALL_PRODUCTS,
      error: null,
    })
    ...
    const response = await GET(
      createRequest("?sort_by=availability&sort_order=asc"),
    )

    expect(response.status).toBe(200)
    const body = await response.json()

    expect(body.products[0].id).toBe("prod-003")
  })
      ...
      docs: [
        {
          data: () => ({
            produit_id: "prod-001",
            images: [{ url: "/products/audio.webp", est_principale: true }],
          }),
        },
      ],
    })
