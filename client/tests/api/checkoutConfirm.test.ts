...
const mockStripePaymentMethodRetrieve = vi.fn()
...
vi.mock("@/lib/stripe/client", () => ({
  getStripeClient: () => ({
    ...
    paymentMethods: {
      retrieve: (id: string) => mockStripePaymentMethodRetrieve(id),
    },
  }),
}))
...
describe("POST /api/checkout/confirm", () => {
  beforeEach(() => {
    ...
    mockFirebaseBucketPublicUrl.mockReturnValue(
      "https://storage.example.com/invoices/test.pdf",
    )
  ...
  it("retourne 201 avec commande créée en cas de succès", async () => {
    ...
    expect(mockStripePaymentMethodRetrieve).toHaveBeenCalledWith(
      "pm_stripe_001",
    )
